...
{
    [InitializeOnLoad]
    public static class ExcelDataReaderInitializer
    {
        static ExcelDataReaderInitializer() => EditorApplication.delayCall += Init;
        
        public static void Init()
        {
            if(!Directory.Exists(Application.dataPath + "/Plugins/ExcelDataReader"))
            {
                string sourcePath = Application.dataPath + "/02.Script/Plugin~/ExcelDataReader";
                ...
                if (!Directory.Exists(sourcePath))
                {
                    ...
                    sourcePath = Path.Combine(packagePath, "Plugin~/ExcelDataReader");
                }
                

                string destinationPath = Application.dataPath + "/Plugins/ExcelDataReader";
